    ...
    <div class="home">
      <div class="header1" :style="{ width: bWidth + 'px' }">
        <div class="title main" :style="{ width: width + 'px' }">
          ...
          <p>专家项目议标评审系统</p>
          <p>
            <span @click="toPage('./personInfo.html')">{{
              userInfo.nickname
            }}</span
      ...
      <div class="personInfo main" :style="{ width: width + 'px' }">
        ...
        <div class="intro">
          ...
          <p class="score">
            合计总分：<span class="red">{{ score }}分</span>
          </p>
        </div>
        <table>
          <thead>
            <tr>
              <td>评定指标</td>
              <td v-for="(item, index) in tableList" :key="'1' + index">
                {{ item.nm || item.svsItemNm }}
              </td>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>评定权重</td>
              <td v-for="(item, index) in tableList" :key="'2' + index">
                {{ item.weight }}%
              </td>
            </tr>
            <tr>
              <td>本项满分得分</td>
              <td v-for="(item, index) in tableList" :key="index + '3'">
                {{ item.fullScore }}
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td>评分说明</td>
              <td v-for="(item, index) in tableList" :key="index + '8'">
                {{ item.tips }}
              </td>
            </tr>
            <tr>
        ...
        <div class="file">
          ...
          <div class="title">
            <img :src="file" />
            <p>附件：</p>
            <span @click="toDown"><img :src="down" />全部下载</span>
          </div>
          <ul>
            <li
...
export default {
  ...
  methods: {
    ...
    toDown() {
      let url =
        "/general/oss/aliDownload?urls=" +
        this.reviewInfo.attachDecode +
        "&zipName=''";
      window.open(url);
