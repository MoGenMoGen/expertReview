		...
		<div class="home">
			<div class="header1" :style="{width:bWidth + 'px'}">
				<div class="title main" :style="{width:width + 'px'}">
					...
					<p>专家项目议标评审系统</p>
					<p><span @click="toPage('./personInfo.html')">{{userInfo.nickname}}</span>|<span
			...
			<div class="personInfo main" :style="{width:width + 'px'}">
				<div class="search">
					...
					<div>
						<p>发布时间：</p>
						<el-date-picker v-model="search.time" type="daterange" range-separator="至"
				...
				<el-table :data="list" header-cell-class-name="tableHeader" cell-class-name="rowList" border
					style="width: 100%">
					<el-table-column type="index" label="序号" align="center" min-width="4%">
					</el-table-column>
					<el-table-column prop="nm" label="项目名称" min-width="25%">
						<template slot-scope="scope">
							<p @click="toDetail(scope.row.id)" class="projectNm">{{scope.row.nm}}</p>
						</template>
					</el-table-column>
					<el-table-column prop="publishTm" label="发布时间" align="center" min-width="10%">
					</el-table-column>
					<el-table-column prop="publisher" label="发布人" align="center" min-width="8%">
					</el-table-column>
					<el-table-column prop="mob" label="联系电话" align="center" min-width="12%">
					</el-table-column>
					...
					<el-table-column prop="status" label="招标采购商" min-width="20%">
						<template slot-scope="scope">
							<ul class="companyList">
								<li v-for="(item,i) in scope.row.offers" :key="i">
									<p :title="item.orgNm">{{item.orgNm}}</p> <span><img
											:src="file" />{{item.fileNums}}</span>
								</li>
							</ul>
						</template>
					</el-table-column>
					<el-table-column label="签到" min-width="8%" align="center">
						<template slot-scope="scope">
							<p style="cursor: pointer;color: #333333;" @click.stop="toSign(scope.row)" v-if="scope.row.signin.shipBidSigninVo.signinStatus==0">签到</p>
							<p v-if="scope.row.signin.shipBidSigninVo.signinStatus==1">已签到</p>
						</template>
					</el-table-column>
				...
				<el-pagination background @current-change="handleCurrentChange" :page-size="pageSize"
					:current-page.sync="pageNo" :total="total" layout="total,prev, pager, next">
				</el-pagination>
			</div>
		</div>
		<my-footer></my-footer>
		...
		methods: {
			...
			async getList() {
				...
				if (this.search.time && this.search.time.length > 0) {
					...
					this.query.toW(qry, 'publishTm', timeS.year + '-' + timeS.month + '-' + timeS.day + ',' + timeE
						.year + '-' + timeE.month + '-' + timeE.day, 'BT')
				}
			...
			toSign(row) {
				this.api.sign(row.signin.shipBidSigninVo.id).then(res => {
					console.log(res)
