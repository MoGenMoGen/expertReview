<template>
	...
	<div style="max-width: 100%">
		<decryptDetail :bidId="bidId" :detailId="detailId" v-if="showDetail"></decryptDetail>

		<div class="title">{{ detail.nm }} (项目编号:{{ detail.cd }})</div>
		<!-- <div class="small_title">标项1 （解密中）</div> -->
		<el-table max-height="524" :data="tableData" style="width: 100%" :cell-style="{
        'text-align': 'center',
        color: '#333',
        'font-weight': '500',
      }" :header-cell-style="{
        ...
        <template slot-scope="scope">
          ...
          <div v-if="scope.row.SCstatus == 1" style="color: 'pink'">已上传</div>
			...
			<el-table-column label="解密文件" prop="attachDecode" min-width="200">
				<template slot-scope="scope">
					<div class="fileList" v-for="(item, index) in scope.row.newList" :key="index">
						<!-- <span> {{ index + 1 }}、 </span> -->
						<!-- <div> -->
							<!-- <img :src="item.img" style="width: 100px; height: 100px; cursor: pointer"
								@click="toLink(item.url)" /> -->
							<p style="cursor: pointer" class="decryptLink" @click="toLink(item.url)">
								{{ item.fileNm }}
							</p>
						<!-- </div> -->
					</div>
            ...
            " @click="handleDecrypt(scope.row.id)">
					...
					<el-button style="color: #2778be" @click="toDetail(scope.row.id)" type="text" size="small">查看
					</el-button>
