		...
		<div class="detailBox">
			...
			<div class="detailContent">
				<el-table :data="tableData" style="width: 100%" :cell-style="{
					...
					<el-table-column prop="options" label="审核意见" min-width="200" v-if="sonTabIndex==1"></el-table-column>
		...
		<div class="mask" @click="closeMask" v-if="showPop">
		  <div class="table_box" @click.stop="">
		    <div class="top">
		      ...
		      <img @click="closeMask" src="~assets/img/close.png" style="width: 25px; height: 25px;cursor: pointer;"/>
		    </div>
		    <div class="row2">
		      <span>{{companyNm}}</span>
		      <el-input v-model="options" type="textarea" :rows="5" resize='none' placeholder="企业审核意见"></el-input>
		    </div>
		    <div class="btn">
		      <el-button style="background: #2778be; color: #fff; margin-right: 20px; padding: 10px 25px; border-radius: 4px;"
		        @click="pass"
		        type="text"
		        size="small">通过</el-button>
		      <el-button style="background: #fff; color: #333; border: 1px solid #dddddd; padding: 10px 25px; border-radius: 4px;"
		        @click="reject"
		        type="text"
